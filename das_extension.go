...
func (fs *FFTSettings) dASFFTExtension(ab []bls.Big, domainStride uint64) {
	if len(ab) == 2 {
		aHalf0 := &ab[0]
		aHalf1 := &ab[1]
		var x bls.Big
		bls.AddModBig(&x, aHalf0, aHalf1)
		var y bls.Big
		bls.SubModBig(&y, aHalf0, aHalf1)
		var tmp bls.Big
		bls.MulModBig(&tmp, &y, &fs.expandedRootsOfUnity[domainStride])
		bls.AddModBig(&ab[0], &x, &tmp)
		bls.SubModBig(&ab[1], &x, &tmp)
		return
	}
	...
	var tmp1, tmp2 bls.Big
	for i := uint64(0); i < halfHalf; i++ {
		aHalf0 := &abHalf0s[i]
		aHalf1 := &abHalf1s[i]
		bls.AddModBig(&tmp1, aHalf0, aHalf1)
		bls.SubModBig(&tmp2, aHalf0, aHalf1)
		bls.MulModBig(aHalf1, &tmp2, &fs.reverseRootsOfUnity[i*2*domainStride])
		bls.CopyBigNum(aHalf0, &tmp1)
	}
	...
	for i := uint64(0); i < halfHalf; i++ {
		...
		bls.CopyBigNum(&x, &abHalf0s[i])
		bls.CopyBigNum(&y, &abHalf1s[i])
		...
		bls.AddModBig(&abHalf0s[i], &x, &yTimesRoot)
		bls.SubModBig(&abHalf1s[i], &x, &yTimesRoot)
	}
}
...
func (fs *FFTSettings) DASFFTExtension(vals []bls.Big) {
	...
	var invLen bls.Big
	bls.AsBig(&invLen, uint64(len(vals)))
	bls.InvModBig(&invLen, &invLen)
	for i := 0; i < len(vals); i++ {
		bls.MulModBig(&vals[i], &vals[i], &invLen)
	}
}
